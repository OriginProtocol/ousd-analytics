...
def get_events(logs):
    """ Get OUSD Governance Voting Settings changed events """
    ...
    for ev in get_events(new_logs):
        if ev.topic_0 == SIG_EVENT_VOTING_DELAY_SET:
            ...
            events.append(event_normal(
                "OUSD Governance voting delay changed   🗳️ 🕖",
                "OUSD Governance voting delay changed from {} blocks to {} blocks".format(
                    old_delay, new_delay
                ),
                log_model=ev
            ))
            ...
            events.append(event_normal(
                "OUSD Governance voting delay changed   🗳️ 🕗",
                "OUSD Governance voting period changed from {} blocks to {} blocks".format(
            ...
            events.append(event_normal(
                "OUSD Governance voting threshold changed   🗳️ 🪙",
                "OUSD Governance voting threshold changed from {} COMP to {} COMP".format(
                    old_human, new_human
                ),
                log_model=ev
            ))
